...
{
	g_ImmediateContext->PSSetShader(g_PixelShader2d, NULL, 0);
}
...
{
	XMMATRIX worldViewProjection = XMMatrixTranspose(world * view * perspective);
...
{
	XMMATRIX worldViewProjection = XMMatrixTranspose(world_view_projection);
...
{
	GetDeviceContext()->UpdateSubresource(g_UVMatrixBuffer, 0, NULL, &matrix, 0, 0);
}
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	depthStencilDesc.DepthEnable = TRUE; //Zテストをするかしないか
	...
	//depthStencilDesc.DepthWriteMask	= D3D11_DEPTH_WRITE_MASK_ZERO; //加算合成に必要（パーティクルなど）
	...
	// 2D用ピクセルシェーダコンパイル・生成
	hr = D3DCompileFromFile(L"shader2D.hlsl", NULL, NULL, "PixelShaderPolygon", "ps_4_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &pPSBlob, &pErrorBlob);
	if (FAILED(hr))
	{
		MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "PS", MB_OK | MB_ICONERROR);
	}

	g_D3DDevice->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), NULL, &g_PixelShader2d);
...

	{
		...
		g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_UVMatrixBuffer);
		g_ImmediateContext->VSSetConstantBuffers(2, 1, &g_UVMatrixBuffer);
	}
	...
	g_ViewMatrix = XMMatrixIdentity(); //行列の内容を初期化

	g_WorldMatrix = XMMatrixIdentity(); //行列の内容を初期化
...
{
	// バックバッファクリア
	float ClearColor[4] = { 0.5f, 0.7f, 0.8f, 1.0f };
